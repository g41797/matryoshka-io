# Matryoshka Zig — Rules (009)

Versioned doc. Replaces [rules-008.md](rules-008.md).
...
- Attached to the example's `pub fn run` — the sole `pub` declaration, and the one
  autodoc actually reads.
...
- `pub fn run` moves to the top of the file, directly after the `///` description +
  diagram block.
- Where a Master exists, its `run` method also moves up, directly after `pub fn run`,
  ahead of struct fields, `init`, `destroy`, and step functions.
...
- `pub fn run` (and Master `run` method, if any) placed at the top of the file, right
...
Master struct shape.
- `run` entry point and the Master's `run` method sit at the top of the file (see
...
- Exception: `examples/` and `stories/` entry-point `pub fn run` — see Description as
...
10. Rules audit: after any stage that changes `*.zig` or `*.md` files, audit all changed files
    ...
    Covers: Observable structural signals, Description as code, Slot Rule, import order,
    banned words, example completeness, Master pattern shape, comment rules, doc rules — all rules.
