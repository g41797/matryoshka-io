# Matryoshka Zig — Rules (022)

Versioned doc. Replaces [rules-021.md](rules-021.md).
...
`examples/`/`stories/` must also update `kitchen/mkdocs.yml`'s Examples
Catalog `nav:` and the matching hand-authored group page under
`kitchen/docs/examples/`. Found right after DOC 20 shipped: the mirrored
`.md` pages generated fine, but none were wired into `nav:`, so mkdocs
silently built them as link-only orphans (`INFO — pages ... not included in
"nav"`) instead of erroring — easy to miss.

Change from rules-019: DOC 20 removed the 8 example-autodoc `zig build docs`
targets (`layer1docs`..`layer4docs`, `itemsdocs`, `hooksdocs`,
`helpersdocs`, `storiesdocs`) and replaced them with a hand-organized
mkdocs examples catalog under `kitchen/docs/examples/`, generated by a new
permanent script (`kitchen/tools/gen_examples_docs.sh`). "Doc-generation
module size" (under Documentation Rules) updated: the general principle
stands, the staging-workaround detail is now historical since the affected
targets no longer exist. See "Doc-generation module size" below.
